...
### v1.1.0 (2026-03-11)
- ✨ **新增功能**
  - 无限滚动加载：优化大量数据展示性能，支持滚动加载更多库位
  - 增强全屏功能：支持 F11 快捷键，跨浏览器兼容，状态同步更准确
  - CSV 导出安全增强：增加 CSV 转义、内存释放、错误处理、带时间戳文件名
- 🔧 **技术优化**
  - 引入 lodash-es 提供节流函数，优化滚动性能
  - 代码格式化和重构，提升可读性
  - 添加完整的错误处理和用户提示
...
- **工具库**: [lodash-es](https://lodash.com/) (节流函数优化性能)
...
本项目仓库中已包含构建好的 `dist` 目录。您可以直接下载仓库，然后用浏览器打开 `dist/index.html` 即可预览完整功能（包含模拟数据）。
